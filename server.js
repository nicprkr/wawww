...
async function searchAICArtist(artistName) {
    try {
        ...
        const response = await fetch(`https://api.artic.edu/api/v1/artists/search?q=${encodedName}&limit=5`);
...
async function fetchAICArtworks(artistId, limit = 20) {
    try {
        // The search endpoints don't work, so fetch artworks and filter by artist_ids
        console.log(`Fetching artworks and filtering by artist ID ${artistId}...`);
        
        try {
            // Fetch artworks with artist_ids field included
            const response = await fetch(`https://api.artic.edu/api/v1/artworks?limit=100&fields=id,title,image_id,artist_ids`);
            ...
            const filteredArtworks = allArtworks.filter(artwork => {
                ...
                return artworkArtistIds.some(id => parseInt(id) === artistIdNum);
            });
            
            console.log(`Found ${filteredArtworks.length} artworks for artist ${artistId}`);
            ...
            // Limit to requested amount and filter for images
            ...
            // Convert AIC image IDs to URLs
            const processedArtworks = artworksToUse.map(artwork => {
                const imageId = artwork.image_id;
                return {
                    title: artwork.title || 'Untitled',
                    thumbnail: `${iiifUrl}/${imageId}/full/300,/0/default.jpg`,
                    original: `${iiifUrl}/${imageId}/full/843,/0/default.jpg`
                };
            });
